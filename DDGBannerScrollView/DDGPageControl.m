 ...
 */
- (void)initialization {
    self.dotViewClass           = [DDGAnimatedDotView class];
...
- (UIView *)generateDotView {
    ...
    if (self.dotViewClass) {
        ...
        if ([dotView isKindOfClass:[DDGAnimatedDotView class]] && self.dotColor) {
            ((DDGAnimatedDotView *)dotView).dotColor = self.dotColor;
        }
    } else {
...
- (void)changeActivity:(BOOL)active atIndex:(NSInteger)index {
    if (self.dotViewClass) {
        DDGAbstractDotView *abstractDotView = (DDGAbstractDotView *)[self.dots objectAtIndex:index];
        if ([abstractDotView respondsToSelector:@selector(changeActivityState:)]) {
            [abstractDotView changeActivityState:active];
        } else {
            NSLog(@"Custom view : %@ must implement an 'changeActivityState' method or you can subclass %@ to help you.", self.dotViewClass, [DDGAbstractDotView class]);
        }
